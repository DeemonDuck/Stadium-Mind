...
├── requirements.txt
├── .env.example
└── tests/              → 67 tests; conftest.py pins mock mode so the suite never hits the network
```
...
- [x] **Hoisted the language list to one constant — then used that to go from 4 languages to 10.** `["English", "Hindi", "Spanish", "French"]` had been hardcoded **three times** in `app.py`, once per language selector, so adding a language meant remembering three places and `"English"` was separately hardcoded as the no-op check and as three function defaults. `SUPPORTED_LANGUAGES` / `DEFAULT_LANGUAGE` now live in `agents/fan_agent.py`, next to the code that actually does the translating — so the UI structurally cannot offer a language the agent was never wired for. This is the payoff of doing the refactor first: **expanding to 10 languages was then a one-line change**, and it lands in all three selectors at once. The 10 aren't arbitrary — English/Spanish/French are the three host nations' languages (USA, Mexico, Canada), German completes FIFA's four official languages, and Portuguese/Arabic/Hindi/Japanese/Korean/Chinese cover the largest travelling-fan and diaspora populations a Cup-scale event actually sees. Two new tests pin the list's contract (default first, no duplicates) and prove the no-op short-circuit fires for English *and nothing else* — a volunteer picking Arabic must not silently get English back.
- [x] Verified after **every** commit above, not just at the end: `ruff check .` clean, `mypy .` clean (now under the stricter setting), **67 tests passing**, every module's `__main__` demo still runs, and the live client still constructs against a real key with both agents' models intact.
